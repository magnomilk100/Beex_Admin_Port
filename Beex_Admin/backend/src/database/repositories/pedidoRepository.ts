...
class PedidoRepository {
  ...
  static async findByClientIdAndRoboId(cliente_id, robo_id, options: IRepositoryOptions) {
    const currentTenant = MongooseRepository.getCurrentTenant(
      options,
    );
    ...
    let record = await MongooseRepository.wrapWithSessionIfExists(
      Pedido(options.database)
      .find({'cliente': Object(cliente_id), 'robo': Object(robo_id), tenant: currentTenant.id}).sort({'dataTermino':-1})
      .populate('cliente')
      .populate('plano')
      .populate('robo')	  
      .populate('indicadoPor'),
